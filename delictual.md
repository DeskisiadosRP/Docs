# 💀 Sistema Delictual

{% hint style="danger" %}
**📢 REQUISITO OBLIGATORIO PARA ROLES ILEGALES:**

Para realizar cualquier rol ilegal, es **OBLIGATORIO** estar conectado en el canal de Discord del servidor de OD de Deskisiados. El incumplimiento de esta norma resultará en sanciones administrativas.
{% endhint %}

{% hint style="warning" %}
**🚗 USO DE VEHÍCULOS Y COLORES EN ACTOS DELICTUALES:**

Es **OBLIGATORIO** utilizar vehículos con el **color correspondiente a la OD** al realizar cualquier acto ilegal. **NO** está permitido el uso de vehículos personales que no tengan el color de la organización ni vehículos que no tengan el color de la organización. El incumplimiento de esta norma resultará en sanciones administrativas.
{% endhint %}

***

### 🚧Actos Delictuales <a href="#actos-delictuales" id="actos-delictuales"></a>

#### 1) Secuestro Civiles. <a href="#id-6-secuestro-civiles-grado-1" id="id-6-secuestro-civiles-grado-1"></a>

* Atracadores permitidos:
  * Civiles: 1 mín - máx 2.
  * O.D: Máximo 10 y Respetar +1.
* Armas permitidas para civiles: Bajo Calibre, Medio Calibre, armas cortopunzantes y bate.
* Tirador: No.
* Refuerzos: No
* LSPD/BCSO conectados: Respetar +1
* Vehículos: Sólo se permitirán vehículos de 4 puertas (excepto autos Deskisiados), el uso de cualquier otro vehículo será sancionado.
* Tiempo de negociaciones: 5 minutos máximo.
* Cooldown: 30 minutos
* En caso de abatir civiles o pinchar ruedas se debe enviar el entorno correspondiente.
  * **Excepción:** En tiroteos espontáneos (tiroteo de la nada) contra civiles o policías, se permite enviar **entorno tardío** describiendo la acción (ej: abatiendo/abriendo fuego).
* Notas Importantes
  * Conectividad +1 policial
  * Todo acto delictual debe llevar su entorno correspondiente.
    * Ej:
      * /entorno Se verían (x) sujetos vestidos de (x color), en un vehículo ( x modelo y color) aparentemente con (x calibre), secuestrando a civiles.
      * /entorno Se verían (x) sujetos vestidos de (x color), en un vehículo ( x modelo y color) aparentemente con (x calibre) pinchando ruedas de vehículos.  * **Entorno tardío:** En tiroteos espontáneos se permite enviar entorno tardío (ej: /entorno Se vería (1) sujeto abriendo fuego contra civil/policía con bajo calibre).  * Está permitido el cateo a LSPD/BSCO pero solo podrán catear (Balas, chalecos dinero limpio y negro).
  * No podrá solicitar DNI o Placa a los oficiales abatidos
  * No se podrá utilizar vehículos tipo Deskisiados.

"En caso de llegal oficales, se podra negociar, no obstante, los rehenes solo podrán ser considerados para negociación mientras se mantengan con vida y en pie. En caso de que alguno de ellos resulte gravemente herido o abatido antes del inicio de la negociación, los oficiales estarán autorizados a intervenir de manera inmediata contra los atracadores al llegar a la zona, siguiendo los procedimientos establecidos para situaciones de alto riesgo."

(Consecuencias, será una sanción para el usuario y dependiendo de las graves un strike o warning a la ODs)

#### 2) Asalto a Civiles. <a href="#id-1-asalto-a-civiles-pandillas-mc-y-civiles-grado-1" id="id-1-asalto-a-civiles-pandillas-mc-y-civiles-grado-1"></a>

* Atracadores máximos permitidos:
  * Civiles: 1 mín - 2 máx.
  * O.D: 1 mín - 4 máx.
* Armas permitidas: Bajo calibre, armas cortopunzantes y bate.
* Refuerzos: No
* LSPD/BCSO Disponibles: Respetar +1
* Vehículos: Sólo se permitirá un vehículo de 4 puertas o motocicletas (excepto categoría Deskisiados), el uso de cualquier otro vehículo será sancionado.
* Tiempo e Intentos: Solo están permitidos 4 asaltos a civiles por dia. (Tienen que ser registrados por bitacora en caso de OD's)

{% hint style="info" %}
Notas Importantes

* Conectividad de +1
* Todo acto delictual debe llevar su entorno correspondiente.
  * Ej:
    * /entorno Se verían (x) sujetos vestidos de (x color), en un vehículo ( x modelo y color) aparentemente con (x calibre) asaltando a civiles.
    * /entorno Se verían (x) sujetos vestidos de (x color), en un vehículo ( x modelo y color) aparentemente con (x calibre) pinchando ruedas de vehículos.
  * **Entorno tardío:** En tiroteos espontáneos se permite enviar entorno tardío (ej: /entorno Se vería (1) sujeto abriendo fuego contra civil/policía con bajo calibre).
* No está permitido el cateo a LSPD/BSCO en ninguna fase.
* No se podrá utilizar vehículos tipo Deskisiados.
* **LSPD/BCSO:** No está permitido el abuso de la animación shield2.
* Está prohibido robar a trabajadores de:
  * Locales del servidor en servicio.
  * Trabajos gubernamentales.
  * Servicio de EMS.

No se podra robar lo siguiente:

* Comida y bebestible.
* Celular o Radio.
* Cosas VIP o Accesorios de armas (revisar el arma antes de irte).
* Accesorios de UWU, ToyStore.
* Solo se podra robar un maximo de $50.000.

"En caso de llegal oficales, se podra negociar, no obstante, los rehenes solo podrán ser considerados para negociación mientras se mantengan con vida y en pie. En caso de que alguno de ellos resulte gravemente herido o abatido antes del inicio de la negociación, los oficiales estarán autorizados a intervenir de manera inmediata contra los atracadores al llegar a la zona, siguiendo los procedimientos establecidos para situaciones de alto riesgo."

(Consecuencias, será una sanción para el usuario y dependiendo de la graves un strike o warning a la ODs)
{% endhint %}

#### 3) Graffiti <a href="#id-12-graffiti-grado-2" id="id-12-graffiti-grado-2"></a>

* Atracadores permitidos:
  * O.D.: 1 mín - 3 máx
* Armas permitidas: Arma Blanca.
* Cantidad por integrante: 2 por dia.
* Tirador: no permitido.
* Rehenes: No
* Refuerzos: No
* LSPD/BCSO conectados: Mínimo 2
* Vehículos: **motocicleta/bicicletas**.
* Tiempo de negociaciones: No
* Cooldown: 5 minutos.
* En caso de abatir civiles, pinchar ruedas o llegar refuerzos se debe enviar el entorno correspondiente.
  * **Excepción:** En tiroteos espontáneos se permite enviar **entorno tardío** describiendo la acción.

{% hint style="info" %}
Notas Importantes

* No se podra catear a LSPD/Sheriff.
* **LSPD/BCSO:** No está permitido el abuso de la animación shield2.
{% endhint %}

#### 4) Parquímetros. <a href="#id-3-parquimetro-pandillas-mc-y-civiles-grado-1" id="id-3-parquimetro-pandillas-mc-y-civiles-grado-1"></a>

* Atracadores permitidos:
  * Civiles: 1 máx.
  * O.D.: 2 máx.
* Armas permitidas: Bajo calibre y armas cortopunzantes.
* Rehenes: No.
* Refuerzos: No
* LSPD/BCSO Conectados: Mínimo 2 (Respetar +1)
* Vehículos: Sólo se permitirá una **motocicleta/bicicletas**, el uso de cualquier otro vehículo será sancionado.
* Cooldown: 15 minutos.

{% hint style="info" %}
Notas Importantes

* Conectividad +1
* Todo acto delictual debe llevar su entorno correspondiente.
  * Ej:
    * /entorno Se verían (x) sujetos vestidos de (x color), en una (motocicleta o bicicleta) ( x modelo y color), posicionándose para robar un parquímetro, este va con (x calibre).
  * **Entorno tardío:** En tiroteos espontáneos se permite enviar entorno tardío.
* No está permitido el cateo a LSPD/BSCO.
* No está permitido el pincho de ruedas para ambas partes.
* No se podrá utilizar vehículos tipo Deskisiados RP.
* **LSPD/BCSO:** No está permitido el abuso de la animación shield2.

(Consecuencias, será una sanción para el usuario y dependiendo de las graves un strike o warning a la ODs)
{% endhint %}

#### 5) Cajero/ATM. <a href="#id-2-cajero-atm-pandillas-mc-y-civiles-grado-1" id="id-2-cajero-atm-pandillas-mc-y-civiles-grado-1"></a>

* Atracadores permitidos:
  * Civiles: 1 máx.
  * O.D.: 2 máx.
* Armas permitidas: Bajo calibre y armas cortopunzantes.
* Rehenes: No.
* Refuerzos: No
* LSPD/BCSO disponible: Mínimo 2 (Respetar +1)
* Vehículos: Sólo se permitirá un vehículo de 4 puertas (el uso de cualquier otro vehículo será sancionado.)
* Cooldown: 15 minutos.
* Tiempo de espera en caso de que no llegue LSPD 10 minutos.

{% hint style="info" %}
Notas Importantes

* Disponibilidad +1
* Todo acto delictual debe llevar su entorno correspondiente.
  * Ej:
    * /entorno Se verían (x) sujetos vestidos de (x color), en un vehículo ( x modelo y color) y van con (x calibre) al parecer están posicionándose para robar un cajero.
    * /entorno Se verían (x) sujetos vestidos de (x color), en un vehículo ( x modelo y color) y van con (x calibre) al parecer están pinchando ruedas de vehículos.
  * **Entorno tardío:** En tiroteos espontáneos se permite enviar entorno tardío.
* Está permitido el cateo a LSPD/BSCO pero solo podrán catear (Balas, chalecos dinero limpio y negro).
* No se podrá utilizar vehículos tipo Deskisiados.
* **LSPD/BCSO:** No está permitido el abuso de la animación shield2.

(Consecuencias, será una sanción para el usuario y dependiendo de las graves un strike o warning a la ODs)
{% endhint %}

#### 6) Badulaque 24/7 <a href="#id-4-badulaque-24-7-pandillas-mc-y-civiles-grado-1" id="id-4-badulaque-24-7-pandillas-mc-y-civiles-grado-1"></a>

* Atracadores permitidos:
  * Civiles: 2 máx.
  * O.D.: 3 máx.
* Armas permitidas: Bajo calibre, armas cortopunzantes y bate.
* Tirador: No permitido.
* Rehenes: 1 máximo.
* Refuerzos: No
* SAPD disponible: Mínimo 3 (Respetar +1)
* Vehículos: Sólo se permitirá un vehículo de 4 puertas o una motocicleta, el uso de cualquier otro vehículo será sancionado.
* Tiempo de negociaciones: 10 minutos máximo.
* Cooldown: 30 minutos.
* Tiempo de espera en caso de que no llegue LSPD o SHERIFF 10 minutos

{% hint style="info" %}
Notas Importantes

* Disponibilidad +1 policial
* Todo acto delictual debe llevar su entorno correspondiente.
* Ej:
  * /entorno Se verían (x) sujetos vestidos de (x color), en un vehículo ( x modelo y color), robando un badulaque con (x calibre).
  * /entorno Se verían (x) sujetos vestidos de (x color), en un vehículo ( x modelo y color), pinchando ruedas de vehículos con (x calibre).
* **Entorno tardío:** En tiroteos espontáneos se permite enviar entorno tardío.
* Está permitido el cateo a LSPD/BSCO pero solo podrán catear (Balas, chalecos dinero limpio y negro).
* No se podrá utilizar vehículos tipo Deskisiados.

"Los rehenes solo podrán ser considerados para negociación mientras se mantengan con vida y en pie. En caso de que alguno de ellos resulte gravemente herido o abatido antes del inicio de la negociación, los oficiales estarán autorizados a intervenir de manera inmediata contra los atracadores al llegar a la zona, siguiendo los procedimientos establecidos para situaciones de alto riesgo."

(Consecuencias, será una sanción para el usuario y dependiendo de las graves un strike o warning a la ODs)
{% endhint %}

#### 7) Recolección/Procesado Drogas (ZR) <a href="#id-8-recoleccion-drogas-grado-1" id="id-8-recoleccion-drogas-grado-1"></a>

* Atracadores permitidos:
  * Civiles: 1 mín - 3 máx.
  * O.D.: Respetar el +1 de conectividad.
* Armas permitidas: Todo tipo de armamento (no sniper).
* Tirador: no permitido .
* Rehenes: no permitido.
* Refuerzos: No
* Oficiales conectados: Mínimo 2 (Respetar +1)
* Vehículos: Todo vehículo de 4 puertas o motos autorizadas(respetando la zona de la ZR ya sea tierra o carretera).

{% hint style="info" %}
Notas Importantes

* Se podrá catear en cualquier fase de inconciencia a OD's y Facción legal.
  * Tienen un plazo de 20 minutos para realizar todas estas acciones y al finalizar se debe dejar a los abatidos fueras de la ZR para que ems llegue a la zona.
* No se puede robar comida.
* Todo acto delictual debe llevar su entorno correspondiente. En este caso se deberan enviar entornos entrada, salida y cosechando/procesando.
  * Ej:
    * /entorno Se verían (x) sujetos vestidos de (x color), en un vehículo ( x modelo y color), entrando a ZR con (x calibre).
    * /entorno Se verían (x) sujetos vestidos de (x color), en un vehículo ( x modelo y color), recolectando/procesando droga con (x calibre).
    * /entorno Se verían (x) sujetos vestidos de (x color), en un vehículo ( x modelo y color), retirandose de zr con (x calibre).
  * **Entorno tardío:** En tiroteos espontáneos se permite enviar entorno tardío.
* Queda prohibido esconderse en arbustos.
* Se podrá catear a LSPD/BCSO únicamente: armamento, munición, chalecos y objetos ilegales, en cualquier fase de inconciencia.
* Se podra solicitar DNI o Placa a los oficiales abatidos
* No se podrán utilizar Vehículos de 2 puertas o Autos Deskisiados.
* **LSPD/BCSO:** No está permitido el abuso de la animación shield2.
{% endhint %}

#### 8) Venta de Drogas. <a href="#id-11-venta-de-drogas-pandillas-mc-y-civiles-grado-1" id="id-11-venta-de-drogas-pandillas-mc-y-civiles-grado-1"></a>

* Atracadores permitidos:
  * Civiles: 1 máx
  * O.D.: 2 máx.
* Armas permitidas: Bajo calibre, armas cortopunzantes y bate.
* Rehenes: No permitido.
* Refuerzos: No
* LSPD/BCSO conectados: Mínimo 1 (respetar +1)
* Vehículos: Sólo se permitirá el uso de una motocicleta, la utilización de cualquier otro tipo de vehículo será sancionado.

{% hint style="info" %}
Notas Importantes

* Conectividad +1
* Entorno automático
* Todo acto delictual debe llevar su entorno correspondiente.
  * Ej:
    * /entorno Se verían (x) sujetos vestidos de (x color), en un vehículo ( x modelo y color), Vendiendo droga con (x calibre).
  * **Entorno tardío:** En tiroteos espontáneos se permite enviar entorno tardío.
* Está permitido el cateo a LSPD/BSCO pero solo podrán catear (Balas, chalecos dinero limpio y negro).
* No podra solicitar DNI o Placa a los oficiales abatidos
* No se podrá utilizar vehículos tipo Deskisiados.
* **LSPD/BCSO:** No está permitido el abuso de la animación shield2.

(Consecuencias, será una sanción para el usuario y dependiendo de las graves un strike o warning a la ODs)
{% endhint %}

#### 9) Banco Fleeca. <a href="#id-13-banco-fleeca-bandas-mafias-y-carteles-grado-2" id="id-13-banco-fleeca-bandas-mafias-y-carteles-grado-2"></a>

* Atracadores permitidos:
  * O.D.: 2 mín - 5 máx
* Armas permitidas: Bajo calibre y Medio calibre (Prohibido el uso de Alto calibre)
* Tirador: No.
* Refuerzos: No
* Rehenes: 2 máximo.
* LSPD/BCSO disponible: Respetar +1
* Vehículos: Sólo se permitirán vehículos de 4 puertas, el uso de cualquier otro vehículo será sancionado.
* Tiempo de negociaciones: 20 minutos máximo.
* Cooldown: 30 minutos.
* Tiempo de espera en caso de que no llegue LSPD o SHERIFF 15 minutos

{% hint style="info" %}
Notas Importantes

* Disponibilidad +1 policial
* Todo acto delictual debe llevar su entorno correspondiente.
  * Ej:
    * /entorno Se verían (x) sujetos vestidos de (x color), en un vehículo ( x modelo y color), robando banco fleca, con bajo calibre.
    * /entorno Se verían (x) sujetos vestidos de (x color), en un vehículo ( x modelo y color), pinchando ruedas de vehículos, con bajo calibre.
  * **Entorno tardío:** En tiroteos espontáneos se permite enviar entorno tardío.
* Está permitido el cateo a LSPD/BSCO pero solo podrán catear (Balas, chalecos dinero limpio y negro).
* No se podrá utilizar vehículos tipo Deskisiados.
* **LSPD/BCSO:** No está permitido el abuso de la animación shield2.

Obstrucciones durante persecuciones

* Solo se podrán realizar obstrucciones durante una persecución si el responsable de efectuar la maniobra ya se encuentra dentro de la zona por donde transitarán los sospechosos.
* La obstrucción deberá ejecutarse únicamente cuando los sospechosos ingresen al área, manteniendo comunicación constante con el equipo y evitando en todo momento poner en riesgo a terceros.
* Para llevar a cabo esta acción será obligatorio enviar un entorno que describa estas condiciones.
  * /entorno Se verían 1 sujeto vestido de (x color), en un vehículo ( x modelo y color), obstruyendo a oficiales, con mele.
* El envío del entorno se considerará como un **corte automático de negociaciones**.
* Quien resalice esta acción no podra incorporarse en el tiroteo posterior, por lo cual debera ir con arma mele.

"Los rehenes solo podrán ser considerados para negociación mientras se mantengan con vida y en pie. En caso de que alguno de ellos resulte gravemente herido o abatido antes del inicio de la negociación, los oficiales estarán autorizados a intervenir de manera inmediata contra los atracadores al llegar a la zona, siguiendo los procedimientos establecidos para situaciones de alto riesgo."

(Consecuencias, será una sanción para el usuario y dependiendo de las graves un strike o warning a la ODs)
{% endhint %}

#### 10) Joyería. <a href="#id-14-joyeria-bandas-mafias-y-carteles-.-grado-2" id="id-14-joyeria-bandas-mafias-y-carteles-.-grado-2"></a>

* Atracadores permitidos
  * O.D.: 4 min- 6 máx.
* Armas permitidas: Medio calibre.
* Tirador: 1 Máximo. (Tiene que estar especificado en el entorno)
* Rehenes: Mínimo 2 – Máximo 3 (no rehenes ficticios) (Se podrá negociar la mitad del botín como máximo para conseguir otro beneficio.)
* Refuerzos: No
* LSPD/BCSO disponible: Respetar +1
* Vehículos: Sólo se permitirán vehículos de 4 puertas, el uso de cualquier otro vehículo será sancionado.
* Tiempo de negociaciones: 10 minutos máximo.
* Cooldown: 30 Mín.
* Los atracadores deberán esperar 20 minutos a que llegue la policía al lugar. En caso contrario, podrán escapar con el respectivo entorno.

> Tirador: Cualquier sujeto posicionado fuera del perímetro/establecimiento en el cual se está realizando el acto delictual y que se encuentra dentro del total de delincuentes realizando el acto delictual, esto no define el arma "Francotirador".

{% hint style="info" %}
Notas Importantes

* Disponibilidad +1 policial
* Todo acto delictual debe llevar su entorno correspondiente.
  * Ej:
    * /entorno Se verían (x) sujetos vestidos de (x color), en un vehículo ( x modelo y color), robando joyeria, con medio calibre.
    * /entorno Se verían (x) sujetos vestidos de (x color), en un vehículo ( x modelo y color), pinchando ruedas de vehículos, con medio calibre.
  * **Entorno tardío:** En tiroteos espontáneos se permite enviar entorno tardío.
* Está permitido el cateo a LSPD/BSCO pero solo podrán catear (Balas, chalecos dinero limpio y negro).
* No se podrá utilizar vehículos tipo Deskisiados.
* No se podrá catear armas no letales.
* **LSPD/BCSO:** Está permitido el uso de la animación shield2.

Obstrucciones durante persecuciones

* Solo se podrán realizar obstrucciones durante una persecución si el responsable de efectuar la maniobra ya se encuentra dentro de la zona por donde transitarán los sospechosos.
* La obstrucción deberá ejecutarse únicamente cuando los sospechosos ingresen al área, manteniendo comunicación constante con el equipo y evitando en todo momento poner en riesgo a terceros.
* Para llevar a cabo esta acción será obligatorio enviar un entorno que describa estas condiciones.
  * /entorno Se verían 1 sujeto vestido de (x color), en un vehículo ( x modelo y color), obstruyendo a oficiales, con mele.
* El envío del entorno se considerará como un **corte automático de negociaciones**.
* Quien resalice esta acción no podra incorporarse en el tiroteo posterior, por lo cual debera ir con arma mele.

"Los rehenes solo podrán ser considerados para negociación mientras se mantengan con vida y en pie. En caso de que alguno de ellos resulte gravemente herido o abatido antes del inicio de la negociación, los oficiales estarán autorizados a intervenir de manera inmediata contra los atracadores al llegar a la zona, siguiendo los procedimientos establecidos para situaciones de alto riesgo."

(Consecuencias, será una sanción para el usuario y dependiendo de las graves un strike o warning a la ODs)
{% endhint %}

#### 11) Venta de armamento <a href="#id-15-robo-a-yate.-bandas-mafias-y-carteles-proximamento-grado-3" id="id-15-robo-a-yate.-bandas-mafias-y-carteles-proximamento-grado-3"></a>

* Atracadores permitidos
  * O.D.: 2 mín. - 5 máx.
* Armas permitidas: Bajo o Medio Calibre.
* Tirador: No.
* Rehenes: No.
* Refuerzos: No.
* LSDP Conectividad: Siempre respetar el +1.
* Vehículos: Vehiculos de 4 puertas o motos.
* Cooldown: 2 horas
* En caso de abatir civiles, pinchar ruedas o llegar refuerzos se debe enviar el entorno correspondiente.
  * **Excepción:** En tiroteos espontáneos se permite enviar **entorno tardío** describiendo la acción.

{% hint style="info" %}
Notas Importantes

* Conectividad +1 policial
* Todo acto delictual debe llevar su entorno correspondiente.
  * Ej:
    * /entorno Se verían (x) sujetos vestidos de (x color), en (x) auto ( x modelo y color), vendiendo armas (especificar calibre), con (x) calibre.
  * **Entorno tardío:** En tiroteos espontáneos se permite enviar entorno tardío.
* El entorno debe ser enviado cada 10 minutos (esto en caso de que la negociacion con el comprador se alarge)
* Está permitido el cateo a LSPD/BSCO pero solo podrán catear (Balas, chalecos dinero limpio y negro).
* La organización delictual tendrá máximo 15 minutos para huir de la zona en caso de ganar el tiroteo.
* Vehiculos de dos puertas y Deskisiados quedan prohibidos.
* **LSPD/BCSO:** No está permitido el abuso de la animación shield2.

Obstrucciones durante persecuciones

* Solo se podrán realizar obstrucciones durante una persecución si el responsable de efectuar la maniobra ya se encuentra dentro de la zona por donde transitarán los sospechosos.
* La obstrucción deberá ejecutarse únicamente cuando los sospechosos ingresen al área, manteniendo comunicación constante con el equipo y evitando en todo momento poner en riesgo a terceros.
* Para llevar a cabo esta acción será obligatorio enviar un entorno que describa estas condiciones.
  * /entorno Se verían 1 sujeto vestido de (x color), en un vehículo ( x modelo y color), obstruyendo a oficiales, con mele.
* El envío del entorno se considerará como un **corte automático de negociaciones**.
* Quien resalice esta acción no podra incorporarse en el tiroteo posterior, por lo cual debera ir con arma mele.

(Consecuencias, será una sanción para el usuario y dependiendo de las graves un strike o warning a la ODs)
{% endhint %}

#### 12) Venta de Cargamento <a href="#id-17-asalto-a-lspd-mc-leyenda-mafias-y-carteles.-grado-3" id="id-17-asalto-a-lspd-mc-leyenda-mafias-y-carteles.-grado-3"></a>

* Atracadores permitidos:
  * O.D.: Toda la OD respetando el +1 de disponibilidad
* Armas permitidas: Medio y alto calibre..
* Tirador: Si. (sniper)
* Rehenes: No
* Refuerzos: No
* BCSO disponible: 9 disponible (respetar +1)
* Vehículos: Vehiculos de 4 puertas o motos.
* Cooldown: 24 horas

{% hint style="info" %}
Notas Importantes

* Conectividad +1 policial.
* Todo acto delictual debe llevar su entorno correspondiente.
  * Ej:
    * /entorno Se verían (x) sujetos vestidos de (x color), en (x) auto ( x modelo y color), vendiendo cargamento, con (x) calibre.
  * **Entorno tardío:** En tiroteos espontáneos se permite enviar entorno tardío.
* Se podrá catear a BCSO únicamente: armamento, munición, chalecos y objetos ilegales, en cualquier fase de inconciencia.
* No se podrá utilizar vehículos tipo Deskisiados.
* Solo se podran vender cargamentos en Norte
* **LSPD/BCSO:** No está permitido el abuso de la animación shield2.

Uso de Francotirador:

* Debe estar posicionado dentro de los 300 Metros correspondientes.
* Para utilizarlo se debe ocupar un /me y un /do correspondiente a ensamblar y desensamblar el armamento.(Entre disparos no es necesario ningún comando)
* Puede ser utilizado con una Pistola como menciona PG en Inventario.
* Este debe llevarse con un bolso (Justificando el peso) y además al utilizar una motocicleta se debe guardar en el bolso (No subirse con este en la mano y hacer la animación de que se guarda).

"Los rehenes solo podrán ser considerados para negociación mientras se mantengan con vida y en pie. En caso de que alguno de ellos resulte gravemente herido o abatido antes del inicio de la negociación, los oficiales estarán autorizados a intervenir de manera inmediata contra los atracadores al llegar a la zona, siguiendo los procedimientos establecidos para situaciones de alto riesgo."

(Consecuencias, será una sanción para el usuario y dependiendo de las graves un strike o warning a la ODs)
{% endhint %}

#### 13) Secuestro a LSPD/BCSO <a href="#id-18-secuestro-a-lspd-mc-leyenda-mafias-y-carteles.-grado-3" id="id-18-secuestro-a-lspd-mc-leyenda-mafias-y-carteles.-grado-3"></a>

* Atracadores permitidos:
  * O.D.: mín 4- 18 máx.
* Armas permitidas: Medio o Alto calibre. (acompañado de una Pistola)
* Tirador: No permitido.
* Refuerzos: No
* LSPD/SHERIFF Disponibles: 9 (Respetar +2)
* Vehículos: Sólo se permitirán vehículos de 4 puertas, el uso de cualquier otro vehículo será sancionado.
* Tiempo de negociaciones: 10 minutos máximo.
* Cooldown: 3 secuestros semanales por Organización Delictual.
* En caso de abatir civiles, pinchar ruedas o llegar refuerzos se debe enviar el entorno correspondiente.
  * **Excepción:** En tiroteos espontáneos se permite enviar **entorno tardío** describiendo la acción.

{% hint style="info" %}
Notas Importantes

* Podrá caer toda la malla policial disponible al lugar del secuestro, sin embargo al momento de la persecución LSPD/BCSO deberá respetar un +2 sobre la cantidad de atracadores presentes.
* Todo acto delictual debe llevar su entorno correspondiente.
  * Ej:
    * /entorno Se verían (x) sujetos vestidos de (x color), en un vehículo ( x modelo y color), secuestrando a LSPD/sheriff, con medio calibre/alto calibre. (Especificar cual)
    * /entorno Se verían (x) sujetos vestidos de (x color), en un vehículo ( x modelo y color), pinchando ruedas de vehículos de LSPD/Sheriff , con medio calibre/alto calibre. (Especificar cual)
  * **Entorno tardío:** En tiroteos espontáneos se permite enviar entorno tardío.
* No se podrá utilizar vehículos tipo Deskisiados.
* Se podrá catear a LSPD/BCSO únicamente: armamento, munición, chalecos y objetos ilegales, en fase 1.
* Máximo de rehenes 2 oficiales por secuestro
* Está permitida la activación de malla completa según corresponda, bajo los límites del FairPlay establecidos.
* NO SE PODRÁ SECUESTRAR LSPD/SHERIFF PARA REHENES DE ALGUN ROBO
* **LSPD/BCSO:** Está permitido el uso de la animación shield2.

Obstrucciones durante persecuciones

* Solo se podrán realizar obstrucciones durante una persecución si el responsable de efectuar la maniobra ya se encuentra dentro de la zona por donde transitarán los sospechosos.
* La obstrucción deberá ejecutarse únicamente cuando los sospechosos ingresen al área, manteniendo comunicación constante con el equipo y evitando en todo momento poner en riesgo a terceros.
* Para llevar a cabo esta acción será obligatorio enviar un entorno que describa estas condiciones.
  * /entorno Se verían 1 sujeto vestido de (x color), en un vehículo ( x modelo y color), obstruyendo a oficiales, con mele.
* El envío del entorno se considerará como un **corte automático de negociaciones**.
* Quien resalice esta acción no podra incorporarse en el tiroteo posterior, por lo cual debera ir con arma mele.

"Los rehenes solo podrán ser considerados para negociación mientras se mantengan con vida y en pie. En caso de que alguno de ellos resulte gravemente herido o abatido antes del inicio de la negociación, los oficiales estarán autorizados a intervenir de manera inmediata contra los atracadores al llegar a la zona, siguiendo los procedimientos establecidos para situaciones de alto riesgo."

(Consecuencias, será una sanción para el usuario y dependiendo de las graves un strike o warning a la ODs)
{% endhint %}

#### 14) Banco Central <a href="#id-21-banco-central-mc-leyenda-mafias-y-carteles.-grado-3" id="id-21-banco-central-mc-leyenda-mafias-y-carteles.-grado-3"></a>

* Atracadores permitidos
  * O.D.: 8 min- 16 máx
* Distribución de delincuentes:
  * Mínimo en el Interior: 8
  * Máximo en el Exterior: 8
* Armas permitidas: Alto Calibre. (acompañado de una Pistola)
* Tirador: 1 Máximo (Tiene que estar especificado en el entorno)
* Rehenes: mínimo 2 – máximo 4 (Se podrá negociar la mitad del botín como máximo para conseguir otro beneficio.)
* Refuerzos: No
* LSPD/BCSO disponible: Mínimo 9 (Respetar +1)
* Vehículos: No están permitidos los vehículos de 2 puertas.
* Tiempo de negociaciones: Máximo 15 minutos de negociación. (Se permite el uso de dados en caso de no encontrar acuerdo mutuo)
* Cooldown: 7 días por OD.

{% hint style="info" %}
Notas Importantes

* Disponibilidad +1 Policial
* Todo acto delictual debe llevar su entorno correspondiente.
  * Ej:
    * /entorno Hola buenas LSPD, soy un camionero y haciendo mi ruta, Acabo de ver a (x cantidad) personas vestidos (x color o forma) portando ALTO CALIBRE, en (x vehiculos), robando BANCO CENTRAL de la ciudad, van entrando con (x rehenes) y se ven muy peligrosos!!. Vengan rápido por favor!. Al parecer (x sujetos en el exterior, de 2 a 6) se fueron a un lugar X entre ellos UN FRACTIRADOR y están posicionados estratégicamente fuera del banco central . (TOTAL x, INTERIOR x, EXTERIOR x)
  * **Entorno tardío:** En tiroteos espontáneos se permite enviar entorno tardío.
* Solo está permitido el uso de un sniper y esta tiene que ser utilizada fuera del banco central.
* No se permite el cateo entre compañeros en ningun momento.
* **SÍ está permitido el cateo a LSPD/BCSO** (armamento, munición, chalecos y objetos ilegales). **NO se puede sacar DNI o PLACA**.
* **LSPD/BCSO:** Está permitido el uso de la animación shield2.
* El o los integrantes que robaron el banco central (BOTÍN) tienen que estar posicionados dentro del banco central OBLIGATORIAMENTE.
* No se podra utilizar la oficina durante el tiroteo.

**Negociable**

* Casco (esta negociación influye para ambos bandos)
* **Salida limpia/libre**: No se obstaculiza la salida del vehículo en el alrededor de 250 metros.
* **Integridad del vehículo**: No se podrá tocar el vehículo es decir nada que afecte su funcionamiento.
* **Segundos de huida o Misisipi**: Desde que se sube al vehículo se iniciará el conteo (3 maximo).
* No tiradores (esta negociación influye a ambas bandos).
* No Helicóptero.
* Se elimina un chaleco.
* Parte o totalidad del botin.

**No Negociable**

* Entrega de implementos policiales (armas, chalecos, vehículos).
* Puerta secundaria (NO NEGOCIABLE)
* Desactivación de protocolos tácticos, como la clave PIT o el código 100.
* Eliminación de antecedentes penales.
* Igualdad de Calibre o cambio de Calibre.
* Está prohibido omitir el aviso de corte de negociación en persecuciones.
* Está prohibido forzar beneficios sin justificación IC o sin desarrollo narrativo válido.

**En caso de huida**

* Esto se puede solicitar al inicio de la negociación (por un rehen) o pasados 30 minutos inicioado el tiroteo. siempre y cuando queden 3 delincuentes en pie (en caso de que la persona que tiene el botin sea abatida, este se dara por perdido).
* Se debera enviar un entorno en caso de que se decida huir durante el tiroteo.
* LSPD debera responder mediante anuncio cuando esten listos para la persecución, ante esto, si LSPD no cuenta con la cantidad de oficiales adecuada (+1) estos podran solicitar a central un oficial adicional.
* No se podra abrir investigacion a los delincuentes que sean abatidos y queden en el lugar, ademas de esto, no se podra llamar a EMS para que los ayude ni a un tercero.

Uso de Francotirador:

* Debe estar posicionado dentro de los Metros correspondientes.
* Para utilizarlo se debe ocupar un /me y un /do correspondiente a ensamblar y desensamblar el armamento.(Entre disparos no es necesario ningún comando)
* Puede ser utilizado con una Pistola como menciona PG en Inventario.
* Este debe llevarse con un bolso (Justificando el peso) y además al utilizar una motocicleta se debe guardar en el bolso (No subirse con este en la mano y hacer la animación de que se guarda).

Tirador: Cualquier sujeto posicionado fuera del perímetro/establecimiento en el cual se está realizando el acto delictual y que se encuentra dentro del total de delincuentes realizando el acto delictual, esto no define el arma "Francotirador".

"Los rehenes solo podrán ser considerados para negociación mientras se mantengan con vida y en pie. En caso de que alguno de ellos resulte gravemente herido o abatido antes del inicio de la negociación, los oficiales estarán autorizados a intervenir de manera inmediata contra los atracadores al llegar a la zona, siguiendo los procedimientos establecidos para situaciones de alto riesgo."

(Consecuencias, será una sanción para el usuario y dependiendo de las graves un strike o warning a la ODs)
{% endhint %}

#### 15) Traslado a Federal (Se puede hacer alianza) <a href="#id-23-traslado-a-federal-se-puede-hacer-alianza-grado-3" id="id-23-traslado-a-federal-se-puede-hacer-alianza-grado-3"></a>

* Atracadores permitidos
  * O.D.: 10 min- sin limite.
* Armas permitidas: Medio y Alto calibre (acompañado de una Pistola)
* Tirador: No.
* Rehenes: No.
* Refuerzos: No.
* LSPD/BCSO Disponibles: 6 mínimo - sin limite.
* Vehículos: Todo vehículo de 4 puertas o motos autorizadas.
* Tiempo de negociaciones: No.
* Cooldown: No.

{% hint style="info" %}
Notas Importantes

* Está permitido el cateo a LSPD/BSCO pero solo podrán catear (Balas, chalecos dinero limpio y negro).
* No se podran utilizar Vehículos de 2 puertas ni categoría Deskisiados.
* **LSPD/BCSO:** Está permitido el uso de la animación shield2.
{% endhint %}

#### 16) Ajuste de cuentas entre OD´S. <a href="#id-23-traslado-a-federal-se-puede-hacer-alianza-grado-3-1" id="id-23-traslado-a-federal-se-puede-hacer-alianza-grado-3-1"></a>

* Atracadores permitidos
  * O.D.: Toda la OD respetando el +1 de Conectividad.
* Armas permitidas: El calibre lo determina la zona, es decir, si es en ciudad con bajo calibre y si es en norte calibre liberado. (no sniper)
* Tirador: No.
* Rehenes: No.
* Refuerzos: No.
* LSPD/BCSO Disponibilidad: 9 mínimo (respetar +1)
* Vehículos: Todo vehículo de 4 puertas o motos autorizadas (respetando la zona, es decir si van por tierra en vehiculos 4x4).
* Tiempo de negociaciones: No.
* Cooldown: No.

{% hint style="info" %}
Notas Importantes

* Todo acto delictual debe llevar su entorno correspondiente.
  *   Ej:

      /entorno Se verían (x) sujetos vestidos de (x color), en (x cantidad) vehículo ( x modelo y color), realizando ajute de cuentas, con (x calibre)
  * **Entorno tardío:** En tiroteos espontáneos se permite enviar entorno tardío.
* No se permite el cateo entre compañeros en ningun momento.
* No está permitido el cateo a LSPD/BSCO.
* No se podran utilizar Vehículos de 2 puertas ni categoría Deskisiados.
* **LSPD/BCSO:** No está permitido el abuso de la animación shield2.
* Al finalizar el rol se podra catear a la OD rival y se podra catear lo siguiente:
  * dinero
  * armas
  * municion
  * chaleco
  * drogas
* Se podra sacar DNI a todos los oponentes.
* Se podra secuestrar solo a 1 oponente y no se podra mantener en custodia por mas de 30 minutos.
* No se podra utilizar ropa tactica (no casto).
* No se podra invadir bajo ningun motivo las HQ.
{% endhint %}

### 17) Ammu-Nation <a href="#id-7-ammu-nation-pandillas-mc-herederos-y-civiles-grado-1" id="id-7-ammu-nation-pandillas-mc-herederos-y-civiles-grado-1"></a>

* Atracadores permitidos:
  * Civiles: 1 mín - 2 máx.
  * ODs: 1 mín - 3 máx.
* Armas permitidas: Bajo calibre.
* Tirador: No permitido.
*   Rehenes: 2 máximo (1 ficticio permitido, en caso de no tener reales)

    Se podrá negociar la mitad del botín como máximo para conseguir otro beneficio.
* Refuerzos: No
* LSPD/BCSO disponible: Mínimo 2 (Respetar +1)
* Vehículos: Sólo se permitirá 1 vehículo de 4 puertas, el uso de cualquier otro vehículo será sancionado.
* Tiempo de negociaciones: 10 minutos máximo.
* Cooldown: 30 minutos.
* Tiempo de espera en caso de que no llegue LSPD o SHERIFF 10 minutos

{% hint style="info" %}
Notas Importantes

* Disponibilidad +1policial
* Todo acto delictual debe llevar su entorno correspondiente.
  * Ej:
    * /entorno Se verían (x) sujetos vestidos de (x color), en un vehículo ( x modelo y color), robando ammunation, con bajo calibre.
    * /entorno Se verían (x) sujetos vestidos de (x color), en un vehículo ( x modelo y color), pinchando ruedas de vehículos con bajo calibre.
  * **Entorno tardío:** En tiroteos espontáneos se permite enviar entorno tardío.
* Está permitido el cateo a LSPD/BSCO pero solo podrán catear (Balas, chalecos dinero limpio y negro).
* No se podrá utilizar vehículos tipo Deskisiados.
* **LSPD/BCSO:** No está permitido el abuso de la animación shield2.
{% endhint %}

"<mark style="color:purple;">Los rehenes solo podrán ser considerados para negociación mientras se mantengan con vida y en pie. En caso de que alguno de ellos resulte gravemente herido o abatido antes del inicio de la negociación, los oficiales estarán autorizados a intervenir de manera inmediata contra los atracadores al llegar a la zona, siguiendo los procedimientos establecidos para situaciones de alto riesgo</mark>."

(<mark style="color:red;">Consecuencias, será una sanción para el usuario y dependiendo de las graves un strike o warning a la ODs</mark>)
