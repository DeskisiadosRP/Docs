# 🟢 Zona Segura

## 🛡️ Espacios Protegidos del Servidor

Las Zonas Seguras son áreas donde la violencia y los conflictos están estrictamente restringidos para garantizar una experiencia de roleplay equilibrada.

***

## 📏 Delimitación

{% hint style="info" %}
**Radio de Protección:** 300 metros a la redonda desde el punto central
{% endhint %}

Estas zonas están protegidas por **RDI (Rol de Interpretación)**, asegurando un ambiente tranquilo para todos los jugadores.

***

## 🏢 Zonas Seguras del Servidor

### 🚔 Instalaciones Policiales

{% hint style="success" %}
**Comisarías de LSPD y Sheriff**
{% endhint %}

#### Propósito:

* Interacciones legales con autoridades
* Realizar denuncias
* Procedimientos policiales
* Consultas legales

***

### 🏥 Hospitales

{% hint style="success" %}
**Centros médicos y EMS**
{% endhint %}

#### Propósito:

* Atención médica y recuperación
* Tratamiento de heridas
* Check-ups médicos
* Servicio de EMS

***

### 🅿️ Garajes Públicos

{% hint style="success" %}
**Estacionamientos de uso común**
{% endhint %}

#### Características:

* Solo garajes **públicos**
* **Excluye** garajes privados
* Zona para aparcar sin riesgo
* Acceso libre para todos

***

### 💼 Puntos de Trabajo

{% hint style="success" %}
**Locales comerciales y lugares de empleo**
{% endhint %}

#### Incluye:

| Tipo                     | Ejemplos                |
| ------------------------ | ----------------------- |
| 🔧 **Mecánicos**         | Talleres y bengalas     |
| 🍔 **Locales de comida** | Restaurantes, fast food |
| ⛽ **Gasolineras**        | Estaciones de servicio  |
| 👔 **Tiendas de ropa**   | Boutiques, tiendas      |
| 💇 **Barberías**         | Salones de belleza      |
| 📦 **Trabajos estándar** | Diversos empleos        |
| 🏁 **Spawn de trabajos** | Puntos de inicio        |
| 🛒 **24/7**              | Tiendas de conveniencia |

***

## ⚖️ Normas en Zona Segura

### ✅ Permitido

* 💬 Roleplay civil tranquilo
* 🤝 Interacciones pacíficas
* 💼 Desarrollo de trabajos
* 🛒 Comercio y compras
* 🚗 Aparcar vehículos

### ❌ Prohibido

{% hint style="danger" %}
**Estrictamente prohibido en estos 300 metros:**\
**Esto cuenta también para todo tipo de locales como de comida y mecánicos.**
{% endhint %}

* 🔫 **Disparos o tiroteos**
* 👊 **Peleas o agresiones**
* 💰 **Robos o asaltos**
* 🚗 **Secuestros**
* ⚔️ **Conflictos entre facciones**
* 🎯 **Persecuciones violentas**

***

## 🚨 ¿Qué Pasa Si Se Viola?

{% hint style="warning" %}
**Violaciones a la Zona Segura resultan en sanciones administrativas**
{% endhint %}

### Sanciones:

| Infracción           | Primera Vez  | Reincidencia   |
| -------------------- | ------------ | -------------- |
| **Disparar en ZS**   | Ban 3-7 días | Ban 15-30 días |
| **Agredir en ZS**    | Ban 1-3 días | Ban 7-15 días  |
| **Robar en ZS**      | Ban 1-3 días | Ban 7-15 días  |
| **Secuestrar en ZS** | Ban 3-7 días | Ban 15-30 días |

***

## 💡 Propósito de las Zonas Seguras

{% hint style="success" %}
**Objetivo principal:**

Permitir que los jugadores puedan:

* 🎭 Desarrollar roles civiles tranquilos
* 💼 Trabajar sin interrupciones
* 🏥 Recibir atención médica
* 👮 Interactuar con autoridades legalmente
* 🛡️ Tener espacios de respiro del conflicto
{% endhint %}

***

## 📍 Mapa de Zonas Seguras

{% hint style="info" %}
**Importante:** Las zonas seguras están claramente marcadas en el mapa del servidor. Si tienes dudas sobre si estás dentro de una ZS, pregunta al staff.
{% endhint %}

***

#### 🛡️ Respeta las Zonas Seguras 🛡️

**Todos merecemos espacios tranquilos para rolear**
